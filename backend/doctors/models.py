...
class ExaminationRoom(BaseModel):
    ...
    type = models.CharField(
        max_length=ENUM_LENGTH["DEFAULT"], choices=[(r.value, r.name) for r in RoomType]
    )
    building = models.CharField(max_length=DOCTOR_LENGTH["BUILDING"])
    floor = models.IntegerField()
    note = models.CharField(
        max_length=DOCTOR_LENGTH["ROOM_NOTE"], blank=True, null=True
    )
...
class Doctor(BaseModel):
    ...
    identity_number = models.CharField(
        max_length=PATIENT_LENGTH["IDENTITY"], unique=True
    )
    ...
    gender = models.CharField(
        max_length=ENUM_LENGTH["DEFAULT"], choices=[(g.value, g.name) for g in Gender]
    )
    address = models.CharField(
        max_length=COMMON_LENGTH["ADDRESS"], blank=True, null=True
    )
    ...
    type = models.CharField(
        ...
        choices=[(d.value, d.name) for d in DoctorType],
    )
...
class Schedule(BaseModel):
    ...
    shift = models.CharField(
        max_length=ENUM_LENGTH["DEFAULT"], choices=[(s.value, s.name) for s in Shift]
    )
    ...
    current_patients = models.IntegerField(
        default=SCHEDULE_DEFAULTS["CURRENT_PATIENTS"]
    )
    ...
    default_appointment_duration_minutes = models.IntegerField(
        default=SCHEDULE_DEFAULTS["APPOINTMENT_DURATION_MINUTES"]
    )
