...
class AuthService:
    ...
    def _cleanup_expired_registrations(cls):
        """Xóa các registration đã hết hạn"""
...
class ResetTokenService:
    ...
    def validate_reset_token(token):
        """Validate reset token và trả về user nếu hợp lệ"""
        current_time = datetime.now()
...
class UserService:
    ...
    def get_user_by_id(self, user_id):
        return self.map_to_user_response(get_object_or_404(User, id=user_id))
    ...
    def edit_user(self, user_id, data):
        ...
        if data.get('email') and user.email != data['email'] and User.objects.filter(email=data['email']).exists():
            raise ValidationError(_("Email đã được sử dụng"))
        if data.get('phone') and user.phone != data['phone'] and User.objects.filter(phone=data['phone']).exists():
            raise ValidationError(_("Số điện thoại đã được sử dụng"))
        ...
        if related_objects:
            raise ValidationError(
                _("Không thể xóa người dùng này vì còn liên kết với: %(objects)s. "
                    "Vui lòng xóa các liên kết này trước.") % {
                    'objects': ', '.join(related_objects)
                }
            )
        
        user.delete()
        return {"message": _("Xóa người dùng thành công")}
    ...
    def force_delete_user(self, user_id):
        ...
        user.delete()
        return {"message": _("Xóa người dùng và tất cả dữ liệu liên quan thành công")}
