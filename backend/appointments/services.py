...
class AppointmentService:
    ...
    def get_appointments_by_patient_id_optimized(
        ...
        queryset = (
            ...
            .select_related("doctor", "schedule")
    ...
    def count_by_schedule_and_slot_start(schedule_id, slot_start):
        return Appointment.objects.filter(
            schedule_id=schedule_id, slot_start=slot_start
        ).count()

    @staticmethod
    def get_appointments_by_schedule_ordered(schedule_id):
        return Appointment.objects.filter(schedule_id=schedule_id).order_by(
            "slot_start"
        )

    @staticmethod
    def get_appointments_by_doctor_and_schedules(
        doctor_id, schedule_ids, page_no=1, page_size=PAGE_SIZE_DEFAULT
    ):
    ...
    def get_available_time_slots(schedule_id):
        ...
        total_duration_minutes = (
            schedule_end_dt - schedule_start_dt
        ).total_seconds() / 60
        max_appointments_by_time = int(
            total_duration_minutes / schedule.default_appointment_duration_minutes
        )
        ...
        while (
            current_slot_start_dt
            + timedelta(minutes=schedule.default_appointment_duration_minutes)
            <= schedule_end_dt
        ):
            ...
            current_slot_start_dt += timedelta(
                minutes=schedule.default_appointment_duration_minutes
            )
...
class ServiceOrderService:
    ...
    def get_orders_by_room_and_status_and_date(room_id, status=None, order_date=None):
        ...
        return ServiceOrder.objects.filter(**filters).order_by(
    ...
    def upload_test_result(order_id, file: UploadedFile):
        ...
        if settings.DEBUG and not file_url.startswith("http"):
            ...
            if settings.MEDIA_URL and not file_url.startswith(settings.MEDIA_URL):
                ...
                file_url = (
                    media_url + cleaned_path.split("media/", 1)[-1]
                    if "media/" in cleaned_path
                    else media_url + cleaned_path
                )
