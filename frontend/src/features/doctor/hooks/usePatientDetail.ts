...
const statusMap: {
    [key: string]: "PENDING" | "CONFIRMED" | "COMPLETED" | "CANCELLED";
} = {
    P: "PENDING",
    C: "CONFIRMED",
    D: "COMPLETED",
...
interface UsePatientDetailReturn {
    ...
    setCurrentPrescriptionId: (value: number | null) => void;
    searchInput: string;
...
export const usePatientDetail = (
    ...
    const fetchPrescription = useCallback(async (appointmentId: number) => {
        try {
            ...
            const pres = await pharmacyService.getCurrentPrescriptionByAppointmentId(
                appointmentId
            );
            if (pres && pres.appointment === appointmentId) {
                setPrescription(pres);
                setCurrentPrescriptionId(pres.id);
                loadExistingPrescription(pres);
            } else {
                setPrescription(null);
                setCurrentPrescriptionId(null);
                loadExistingPrescription(null);
            }
        } catch (error) {
    ...
    const fetchAppointmentNotes = useCallback(async (appointmentId: number) => {
        try {
            ...
            const notes = await appointmentNoteService.getNotesByAppointmentId(
                appointmentId
            );
    ...
    const createAppointmentNote = useCallback(
        async (appointmentId: number, note: CreateAppointmentNoteRequest) => {
            try {
                setSaving(true);
                const newNote = await appointmentNoteService.createNote(
                    appointmentId,
                    note
                );
    ...
    const updateAppointmentNote = useCallback(
        async (noteId: number, note: CreateAppointmentNoteRequest) => {
            try {
                setSaving(true);
                const updatedNote = await appointmentNoteService.updateAppointmentNote(
                    noteId,
                    note
                );
                setAppointmentNotes((prev) =>
                    prev.map((n) => (n.noteId === noteId ? updatedNote : n))
                );
    ...
    const deleteAppointmentNote = useCallback(
        async (appointmentId: number, noteId: number) => {
            try {
                setSaving(true);
                await appointmentNoteService.deleteAppointmentNote(
                    appointmentId,
                    noteId
                );
    ...
    const updatePatientInfo = useCallback(
        async (appointmentId: number, updateData: UpdatePatientRequest) => {
            try {
                setSaving(true);
                const updated = await patientService.updatePatientInfo(
                    appointmentId,
                    updateData
                );
    ...
    const updateVitalSigns = useCallback(
        async (appointmentId: number, vitalSigns: any) => {
            try {
                setSaving(true);
                const updated = await patientService.updateVitalSigns(
                    appointmentId,
                    vitalSigns
                );
                setPatientDetail((prev) => (prev ? { ...prev, ...updated } : null));
    ...
    const searchMedicines = useCallback(
        async (searchTerm: string): Promise<Medicine[]> => {
            try {
    ...
    const savePrescription = useCallback(
        async (prescriptionData: any): Promise<Prescription | null> => {
            if (!initialAppointmentId) return null;
            try {
                setSaving(true);
                const saved = await pharmacyService.createPrescription(
                    initialAppointmentId,
                    prescriptionData
                );
                setPrescription(saved);
                setCurrentPrescriptionId(saved.id);
    ...
    const loadExistingPrescription = useCallback(
        (prescription: Prescription | null) => {
            if (prescription) {
                ...
                setMedications(
                    prescription.prescription_details.map((detail) => ({
                        ...detail,
                        id: detail.id, // Đảm bảo giữ id từ API
                        medicine_id: detail.medicine.medicine_id,
                        medicine: detail.medicine,
                        prescriptionNotes: detail.prescription_notes,
                    })) || []
                );
            } else {
                setMedications([]);
    ...
    const checkExistingPrescription =
        useCallback(async (): Promise<Prescription | null> => {
            if (!initialAppointmentId) return null;
            try {
                const existing =
                    await pharmacyService.getCurrentPrescriptionByAppointmentId(
                        initialAppointmentId
                    );
    ...
    const refreshSpecific = useCallback(
        (
            ...
            if (targets.includes("patient"))
                promises.push(fetchPatientDetail(appointmentId));
            if (targets.includes("prescription"))
                promises.push(fetchPrescription(appointmentId));
            if (targets.includes("services"))
                promises.push(fetchServiceOrders(appointmentId));
            if (targets.includes("notes"))
                promises.push(fetchAppointmentNotes(appointmentId));
