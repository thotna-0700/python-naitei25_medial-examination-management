...
export const pharmacyService = {
  ...
  async getCurrentPrescriptionByAppointmentId(
    ...
    try {
      ...
      console.error(
        `[${new Date().toLocaleString(
          "vi-VN"
        )}] Error fetching prescription for appointment ${appointmentId}:`,
        error
      );
  ...
  async getPrescriptionHistoryByPatientId(
    ...
    try {
      ...
      console.error(
        `[${new Date().toLocaleString(
          "vi-VN"
        )}] Error fetching prescription history for patient ${patientId}:`,
        error
      );
  ...
  async updatePrescription(
    ...
    try {
      ...
      console.error(
        `[${new Date().toLocaleString(
          "vi-VN"
        )}] Error updating prescription ${prescriptionId}:`,
        error
      );
      throw error;
    }
  ...
  async deletePrescription(prescriptionId: number): Promise<void> {
    try {
      ...
      console.error(
        `[${new Date().toLocaleString(
          "vi-VN"
        )}] Error deleting prescription ${prescriptionId}:`,
        error
      );
  ...
  async getAllMedicines(): Promise<Medicine[]> {
    try {
      ...
      console.error(
        `[${new Date().toLocaleString("vi-VN")}] Error fetching all medicines:`,
        error
      );
  ...
  async searchMedicine(name?: string, category?: string): Promise<Medicine[]> {
    try {
      ...
      const response = await api.get<Medicine[]>("/medicines/search", {
        params,
      });
      return response.data;
    } catch (error) {
      console.error(
        `[${new Date().toLocaleString(
          "vi-VN"
        )}] Error searching medicine with name ${name} and category ${category}:`,
        error
      );
