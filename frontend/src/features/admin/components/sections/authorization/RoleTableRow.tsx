...
export default function RoleTableRow({ role, onViewPermissions, t }: RoleTableRowProps) {
    return (
        <TableRow key={role.id}>
            ...
            <TableCell className="py-3">
                <Badge size="sm" color="base">
                    {role.userCount} {t("authorization.people")}
                </Badge>
            </TableCell>
            <TableCell className="py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                {role.permissions.length} {t("authorization.permissions").toLowerCase()}
            </TableCell>
            <TableCell className="py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                {role.updatedAt}
            </TableCell>
            <TableCell className="py-3">
                <div className="flex gap-2">
                    <button
                        ...
                        title={t("authorization.viewPermissions")}
                    >
                        <Shield size={14} />
                        {t("authorization.assignPermissions")}
                    </button>
                </div>
            </TableCell>
        </TableRow>
